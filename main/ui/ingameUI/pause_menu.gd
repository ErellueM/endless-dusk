...
func update_stats(player):
	...
	var armor_col = (
		Color.GREEN if player.armor > 0 else (Color.RED if player.armor < 0 else Color.WHITE)
	)
	add_stat_row(
		"Armor", get_clean_text(player.armor), UpgradeDatabase.stat_icons["armor"], armor_col
	)

	var rec_text = get_clean_text(player.recovery) + "/sec"
	var rec_color = (
		Color.GREEN if player.recovery > 0 else (Color.RED if player.recovery < 0 else Color.WHITE)
	)
	add_stat_row("Recovery", rec_text, UpgradeDatabase.stat_icons["recovery"], rec_color)

	var might_bonus = (player.might - 1.0) * 100
	var might_col = (
		Color.GREEN if might_bonus > 0 else (Color.RED if might_bonus < 0 else Color.WHITE)
	)
	add_stat_row(
		"Might", get_clean_text(might_bonus) + "%", UpgradeDatabase.stat_icons["might"], might_col
	)
	...
	add_stat_row(
		"Area", get_clean_text(area_bonus) + "%", UpgradeDatabase.stat_icons["area"], area_col
	)
	...
	add_stat_row(
		"Attack Speed",
		get_clean_text(as_bonus) + "%",
		UpgradeDatabase.stat_icons["attack_speed_bonus"],
		as_col
	)
	...
	add_stat_row(
		"Luck", get_clean_text(luck_bonus) + "%", UpgradeDatabase.stat_icons["luck"], luck_col
	)
	...
	add_stat_row(
		"Magnet",
		get_clean_text(mag_bonus) + "%",
		UpgradeDatabase.stat_icons["magnet_mult"],
		mag_col
	)

	var growth_bonus = (player.growth - 1.0) * 100
	var growth_col = (
		Color.GREEN if growth_bonus > 0 else (Color.RED if growth_bonus < 0 else Color.WHITE)
	)
	add_stat_row(
		"Growth",
		get_clean_text(growth_bonus) + "%",
		UpgradeDatabase.stat_icons["growth"],
		growth_col
	)
	...
	if weapons_manager:
		...
		for data in weapon_data_list:
			...
			if data["is_utility"]:
				...
				_add_sleek_weapon_row(
					final_name_bb, "Utility Aura", data["icon"], info_color, 0.0, true
				)
			else:
				...
				_add_sleek_weapon_row(
					final_name_bb, w_info, data["icon"], info_color, fill_ratio, false
				)
