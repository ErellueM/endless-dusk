...
func process_movement(delta: float):
	...
	if is_shooting:
		...
		if distance > actual_attack_range:
			...
			direction = global_position.direction_to(player.global_position).rotated(
				(PI / 2) * strafe_direction
			)
...
func shoot():
	...
	if projectile_scene:
		...
		tween.tween_property(proj, "scale", Vector2.ONE, proj_charge_time).set_trans(
			Tween.TRANS_BACK
		)
