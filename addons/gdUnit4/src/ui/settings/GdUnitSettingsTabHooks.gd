...
func _draw_hook_item(item: TreeItem, rect: Rect2) -> void:
	...
	_hooks_tree.draw_string(
		font, text_pos, hook_name, HORIZONTAL_ALIGNMENT_LEFT, -1, 14, text_color
	)
...
func _on_select_hook_dlg_confirmed() -> void:
	...
	var result := GdUnitTestSessionHookService.instance().load_hook(
		_select_hook_dlg.get_current_path()
	)
	if result.is_error():
...
func _on_btn_delete_hook_pressed() -> void:
	if _selected_hook_item != null:
		...
		GdUnitTestSessionHookService.instance().unregister(_get_hook(_selected_hook_item))
...
func _on_btn_move_up_pressed() -> void:
	...
	GdUnitTestSessionHookService.instance().move_before(
		_get_hook(_selected_hook_item), _get_hook(prev)
	)
	_update_hook_buttons()
...
func _on_btn_move_down_pressed() -> void:
	...
	GdUnitTestSessionHookService.instance().move_after(
		_get_hook(_selected_hook_item), _get_hook(next)
	)
	_update_hook_buttons()
