...
func fire_test_suite_skipped(context: GdUnitExecutionContext) -> void:
	...
	fire_event(
		GdUnitEvent.new().suite_before(
			context.get_test_suite_path(), test_suite.get_name(), skip_count
		)
	)
	...
	var report := GdUnitReport.new().create(
		...
		-1,
		GdAssertMessages.test_suite_skipped(test_suite.__skip_reason, skip_count)
	)
	fire_event(
		GdUnitEvent.new().suite_after(
			context.get_test_suite_path(), test_suite.get_name(), statistics, [report]
		)
	)
	await (Engine.get_main_loop() as SceneTree).process_frame
...
func fire_test_skipped(context: GdUnitExecutionContext, skip_count := 1) -> void:
	...
	var report := GdUnitReport.new().create(
		...
		GdAssertMessages.test_skipped("Skipped from the entire test suite")
	)
	fire_event(
