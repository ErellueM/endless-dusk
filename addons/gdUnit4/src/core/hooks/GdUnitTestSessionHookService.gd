...
static func instance() -> GdUnitTestSessionHookService:
	return GdUnitSingleton.instance(
		"GdUnitTestSessionHookService",
		func() -> GdUnitTestSessionHookService:
...
func load_hook(hook_resourc_path: String) -> GdUnitResult:
	...
	if script.get_base_script() != GdUnitTestSessionHook:
		return GdUnitResult.error(
			"The hook '%s' must inhertit from 'GdUnitTestSessionHook'." % hook_resourc_path
		)
...
func enable_hook(hook: GdUnitTestSessionHook, enabled: bool) -> void:
	...
	GdUnitSignals.instance().gdunit_message.emit(
		"Session hook '{name}' {enabled}.".format(
			{"name": hook.name, "enabled": "enabled" if enabled else "disabled"}
		)
	)
	save_hock_setttings()
...
func register(hook: GdUnitTestSessionHook, enabled: bool = true) -> GdUnitResult:
	if find_custom(hook) != -1:
		return GdUnitResult.error(
			"A hook instance of '%s' is already registered." % hook.get_script().resource_path
		)
...
func unregister(hook: GdUnitTestSessionHook) -> GdUnitResult:
	...
	if hook_index == -1:
		return GdUnitResult.error(
			"The hook instance of '%s' is NOT registered." % hook.get_script().resource_path
		)
...
func execute(hook_func: String, session: GdUnitTestSession, reverse := false) -> GdUnitResult:
	...
	for hook_index in enigne_hooks.size():
		...
		if OS.is_stdout_verbose():
			GdUnitSignals.instance().gdunit_message.emit(
				"Session hook '%s' > %s()" % [hook.name, hook_func]
			)
		var result: GdUnitResult = await hook.call(hook_func, session)
		if result == null:
			failed_hook_calls.push_back(
				GdUnitResult.error("Result is null! Check '%s'" % hook.get_script().resource_path)
			)
	...
	var errors := failed_hook_calls.map(
		func(result: GdUnitResult) -> String:
...
func load_hook_settings() -> void:
	...
	for hock_path: String in hooks_resource_paths.keys():
		...
		var existing_hook: GdUnitTestSessionHook = (
			enigne_hooks
			. filter(
				func(element: GdUnitTestSessionHook) -> bool:
