...
static func instance() -> GdUnitCommandHandler:
	return GdUnitSingleton.instance(
		"GdUnitCommandHandler", func() -> GdUnitCommandHandler: return GdUnitCommandHandler.new()
	)
...
func _do_process() -> void:
	...
	if (
		test_session_command._is_debug
		and test_session_command.is_running()
		and not EditorInterface.is_playing_scene()
	):
		if GdUnitSettings.is_verbose_assert_warnings():
			print_debug(
				"Test Runner scene was stopped manually, force stopping the current test run!"
			)
...
func command_icon(command_id: String) -> Texture2D:
	if not _commnand_mappings.has(command_id):
		push_error(
			"GdUnitCommandHandler:command_icon(): No command id '%s' is registered." % command_id
		)
		print_stack()
		return
...
func command_shortcut(command_id: String) -> Shortcut:
	if not _commnand_mappings.has(command_id):
		push_error(
			(
				"GdUnitCommandHandler:command_shortcut(): No command id '%s' is registered."
				% command_id
			)
		)
		print_stack()
...
func command_execute(...parameters: Array) -> void:
	if parameters.is_empty():
		push_error(
			"Invalid arguments used on CommandHandler:execute()! Expecting [<command_id, args...>]"
		)
		print_stack()
		return
	...
	if not _commnand_mappings.has(command_id):
		push_error(
			"GdUnitCommandHandler:command_execute(): No command id '%s' is registered." % command_id
		)
		print_stack()
		return
...
func _register_command(command: GdUnitBaseCommand) -> void:
	...
	if _commnand_mappings.has(command.id):
		push_error(
			(
				"GdUnitCommandHandler:_register_command(): Command with id '%s' is already registerd!"
				% command.id
			)
		)
		return
	...
	if Engine.is_editor_hint():
		...
		EditorInterface.get_command_palette().add_command(
			...
			"GdUnit4/" + command.id,
			command.execute,
			command.shortcut.get_as_text() if command.shortcut else "None"
		)
