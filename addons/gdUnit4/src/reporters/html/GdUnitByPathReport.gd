...
static func sort_reports_by_path(report_summaries: Array[GdUnitReportSummary]) -> Dictionary:
	...
	for report in report_summaries:
		...
		var suite_report: Array[GdUnitReportSummary] = by_path.get(
			suite_path, [] as Array[GdUnitReportSummary]
		)
...
func write(report_dir: String) -> String:
	...
	var template := GdUnitHtmlPatterns.load_template(
		"res://addons/gdUnit4/src/reporters/html/template/folder_report.html"
	)
...
func apply_testsuite_reports(
	...
	for report: GdUnitTestSuiteReport in test_suite_reports:
		var report_link := (
			GdUnitHtmlReportWriter
			. create_output_path(report_dir, report.path(), report.name())
			. replace(report_dir, "..")
		)
		@warning_ignore("return_value_discarded")
